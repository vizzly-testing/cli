...
2. **Dev Mode** - Local development server with visual testing, config management, and auth
...
- Interactive dashboard at `http://localhost:47392`
  ...
  - Edit configuration
  - Manage authentication and project mappings
- Fast iteration cycle
- Command: `vizzly dev start` or `vizzly dev run "npm test"`
...
1. Developer runs `vizzly dev start` → Background dev server starts
...
4. Dashboard at `http://localhost:47392` shows:
   ...
   - Settings → Edit config (threshold, ports, etc.)
   - Projects → Login, manage project mappings
5. Baselines updated → Tests pass on next run

**One-off Dev Run:**
1. Developer runs `vizzly dev run "npm test"` → Ephemeral server starts
...
**Baseline Management:**
- Dev mode: Baselines stored in `.vizzly/baselines/` as PNG files
...
**Auto-Discovery:**
The client SDK automatically discovers a running dev server by searching for `.vizzly/server.json`
...
- **Live mode**: Dev server serves dashboard that polls for updates (live comparisons, settings, projects)
- **Static mode**: Self-contained HTML report with embedded data (comparisons only, generated by `vizzly dev run`)
