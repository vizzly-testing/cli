...
module('Integration | Component | FormField', function (hooks) {
  ...
  test('it renders text input', async function (assert) {
    ...
    assert
      .dom('[data-test-form-field="username"] .form-label')
      .hasText('Username');
    assert
      .dom('[data-test-form-field="username"] .form-hint')
      .hasText('Must be at least 3 characters');
  ...
  test('it renders with error state', async function (assert) {
    ...
    assert
      .dom('[data-test-form-field="email"] .form-error')
  ...
  test('it renders various input types', async function (assert) {
    await render(
      <template>
        <div
          style="padding: 20px; max-width: 400px; display: flex; flex-direction: column; gap: 16px;"
        >
          <FormField
            @name="text"
            @label="Text Input"
    ...
    assert
      .dom('[data-test-form-field="text"] input')
      .hasAttribute('type', 'text');
    assert
      .dom('[data-test-form-field="password"] input')
      .hasAttribute('type', 'password');
    assert
      .dom('[data-test-form-field="email-field"] input')
      .hasAttribute('type', 'email');
    assert
      .dom('[data-test-form-field="number"] input')
