...
export function OnionSkinMode({
  ...
  return (
    <ComparisonContainer containerClassName="cursor-col-resize">
      <div className="relative inline-block">
        ...
        >
          <ImageWithErrorBoundary
            ...
            style={{
              width:
                sliderPosition > 0 ? `${10000 / sliderPosition}%` : '10000%',
              maxWidth: 'none',
            }}
            onError={onImageError}
            onLoad={onImageLoad}
        ...
        >
          ...
          <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-4 h-4 bg-[var(--accent-brand)] rounded-full shadow-lg border-2 border-[var(--text-primary)]" />
        </div>
      ...
      {/* Labels */}
      <div className="absolute top-2 left-2 bg-black/70 text-[var(--text-primary)] px-2 py-1 rounded text-xs z-10">
        Baseline
      </div>
      <div className="absolute top-2 right-2 bg-black/70 text-[var(--text-primary)] px-2 py-1 rounded text-xs z-10">
        Current
      </div>

      {/* Progress indicator */}
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 bg-black/70 text-[var(--text-primary)] px-3 py-1 rounded text-xs z-10">
