    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(this.port, error => {
        if (error) {
          ...
          logger.debug(`HTTP server listening on port ${this.port}`);
          resolve();
        }
      });
    ...
    try {
      ...
      if (this.tddMode && this.tddService) {
        ...
        if (comparison.status === 'failed') {
          // Visual difference detected - fail immediately
          logger.warn(`❌ TDD: Visual difference detected for ${name}`);
        ...
        if (comparison.status === 'error') {
          // Comparison error
          logger.error(
            `❌ TDD: Comparison failed for ${name}: ${comparison.error}`
          );
          res.statusCode = 500;
          res.end(
            JSON.stringify({
