...
export class TestRunner extends BaseService {
  ...
  async run(options) {
    ...
    // If no token is allowed, just run the command without Vizzly integration
    if (allowNoToken && !this.config.apiKey) {
      const env = {
        ...process.env,
    ...
    try {
      ...
      if (tdd) {
        this.logger.debug('Starting TDD service...');
        await this.tddService.start();
        this.logger.debug('TDD service started successfully');
      }
