...
import { configCommand, validateConfigOptions } from '../../src/commands/config-cmd.js';
...
describe('commands/config', () => {
  ...
  describe('configCommand', () => {
    it('outputs full config as JSON', async () => {
      ...
      await configCommand(null, {}, { json: true }, {
        loadConfig: async () => ({
    ...
    it('outputs specific key as JSON', async () => {
      ...
      await configCommand('comparison.threshold', {}, { json: true }, {
        loadConfig: async () => ({
          server: { port: 47392 },
    ...
    it('returns error for unknown key', async () => {
      ...
      await configCommand('unknown.key', {}, { json: true }, {
        loadConfig: async () => ({
          server: { port: 47392 },
        }),
        getProjectMapping: async () => null,
    ...
    it('includes config file path', async () => {
      ...
      let dataCall = output.calls.find(c => c.method === 'data');
      assert.ok(dataCall);
      assert.strictEqual(dataCall.args[0].configFile, '/path/to/vizzly.config.js');
    });
    ...
    it('masks API token in output', async () => {
      ...
      assert.strictEqual(dataCall.args[0].config.api.tokenPrefix, 'vzt_supe...');
      // Ensure full token is not exposed
      assert.ok(!JSON.stringify(dataCall.args[0]).includes('supersecrettoken12345'));
    });
  });
});
